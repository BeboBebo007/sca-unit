...
from reusable_service.service import (
...
def process_assessment_request(
    ...
    audit_log = payload.get(
        "audit_log",
        "evidence/reusable_service_audit.jsonl",
    )
...
class ReusableRequestHandler(BaseHTTPRequestHandler):
    server_version = "ReusablePythonService/0.2"
    ...
    def do_GET(self) -> None:
        if self.path == "/health":
            self._send_json(
                ...
                {
                    "status": "ok",
                    "service": "Reusable Python Service",
...
def run_local_service(
    ...
    server = ThreadingHTTPServer(
        (host, port),
        ReusableRequestHandler,
    )

    print(
        f"Reusable Python service running at "
        f"http://{host}:{port}"
    )
